...
layui.config({
    ...
    var colWidth=($(window).width()-30)/20;
    //桌面显示提示消息的函数
    ...
    table.render({
        ...
        limit: 10,
        cols: [[
            { field: 'id', type: 'checkbox',width:colWidth },
            { field: 'icon', title: '图标',width:3*colWidth,sort:true},
            { field: 'name', title: '名称' ,width:3*colWidth},
            { field: 'title', title: '标题',width:3*colWidth },
            { field: 'pageURL', title: '页面地址',width:10*colWidth },
            { field: 'openType', title: '页面类型', templet: '#openTypeTpl',width:3*colWidth },
            { field: 'isNecessary', title: '系统菜单', templet: '#isNecessary',width:2*colWidth },
            { field: 'order', title: '排序',width:2*colWidth },
            { title: '操作', fixed: 'right', align: 'center', toolbar: '#barMenu',width:4*colWidth }
        ]]
    });
    //监听工具条
