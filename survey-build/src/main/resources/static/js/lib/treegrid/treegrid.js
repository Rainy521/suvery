TreeGrid = function(_config){
	...
	drowHeader = function(){
		...
		for(i=0;i<cols.length;i++){
			var col = cols[i];
			if(col.folderHidden){
				s += "<th hidden='true' align='" + (col.headerAlign || _config.headerAlign || "center") + "' width='" + (col.width || "") + "'>" + (col.headerText || "") + "</th>";
			}else{
	...
	drowRowData = function(_rows, _cols, _level, _pid){
		...
		for(var i=0;i<_rows.length;i++){
			...
			s += "<tr id='TR" + id + "' pid='" + ((_pid=="")?"":("TR"+_pid)) + "' value='" + open + "' data=\"" + TreeGrid.json2str(row) + "\" rowIndex='" + rownum++ + "' style='display:" + dispaly + "'>";
			for(var j=0;j<_cols.length;j++){
				var col = _cols[j];
				if(col.folderHidden){
					s += "<td hidden='true' align='" + (col.dataAlign || _config.dataAlign || "left") + "'";
				}else{
				...
				if(j==folderColumnIndex){
					s += " style='text-indent:" + (parseInt((_config.indentation || "20"))*(_level-1)) + "px;text-align:left;'> ";
				}else{
					s += ">";
				}
	...
	this.show = function(){
		...
		s += "<table class='table table-bordered table table-hover' id='" + this.id + "' cellspacing=0 cellpadding=0 width='" + (_config.width || "100%") + "' class='TreeGrid'>";
		drowHeader();
		drowData();
		s += "</table>";
	...
	init = function(){
		...
		__root.find("img[folder='Y']").bind("click", function(){
			...
			var isOpen = __root.find("#" + trid).attr("value");
			isOpen = (isOpen == "Y") ? "N" : "Y";
			__root.find("#" + trid).attr("value", isOpen);
	...
	hideSubs = function(_trid){
		var isOpen = __root.find("#" + _trid).attr("value");
	...
	this.getSelectedItem = function(){
		console.log(TreeGrid.str2json(__selectedData));
...
TreeGrid.DEFAULT_LEAF_ICON = "img/defaultLeaf.gif";
TreeGrid.COUNT = 1;
